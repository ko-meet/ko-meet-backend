...
public class UserController {
    ...
    public ResponseEntity<UserSignInResponse> signIn(
            @RequestHeader("latitude") Double latitude,
            @RequestHeader("longitude") Double longitude,
            @Valid @RequestBody UserSignInRequest userSignInRequest) {

        CompletableFuture<Pair<String, String>> currentLocation =
                geocoderService.getCountry(latitude, longitude);

        UserSignInDto userSignInDto =
                userSignInService.signIn(userSignInRequest, currentLocation);
